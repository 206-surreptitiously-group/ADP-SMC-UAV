...
if __name__ == '__main__':
    ...
    testPath0 = os.path.dirname(os.path.abspath(__file__)) + '/../../datasave/nets/position_tracking/pos_new' + s + '/'
    testPath_smc = None
    ...
    if CREATE_FILE:
        ...
        if CONTROLLER == 'RL':
            (pd.DataFrame({'r': r, 'att_out': att_out, 'pos_out': pos_out}).
             to_csv('./uniform_comparative_pos/uniform_mc_test_rl_pos_new' + s + '.csv', sep=',', index=False))
        else:
            (pd.DataFrame({'r': r, 'att_out': att_out, 'pos_out': pos_out}).
