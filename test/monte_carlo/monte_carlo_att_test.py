...
if __name__ == '__main__':
	...
	env_test_smc = uav_att_ctrl_RL(uav_param, att_ctrl_param)
	env_test_smc.reset_uav_att_ctrl_RL_tracking(random_trajectroy=False, yaw_fixed=False, new_att_ctrl_param=att_ctrl_param)
	...
	if CREATE_FILE:
		...
		for i in range(TEST_NUM):
			...
			env_test_smc.reset_uav_att_ctrl_RL_tracking(random_trajectroy=True,
														yaw_fixed=False,
														new_att_ctrl_param=att_ctrl_param,
														outer_param=[A, T, phi0])
			test_r = 0.
			att_out = 0
			while not env_test_smc.is_terminal:
				...
				_rhod, _dot_rhod, _, _ = ref_inner(env_test_smc.time,
												   env_test_smc.ref_att_amplitude,
												   env_test_smc.ref_att_period,
												   env_test_smc.ref_att_bias_a,
												   env_test_smc.ref_att_bias_phase)
				_torque = env_test_smc.att_control(_rhod, _dot_rhod, None)
				env_test_smc.step_update([_torque[0], _torque[1], _torque[2]])
				test_r += env_test_smc.reward

				# env_test_smc.att_image = env_test.att_image_copy.copy()
				# env_test_smc.draw_att(_rhod)
				# env_test_smc.show_att_image(iswait=False)
			r_4_smc.append(test_r)
			att_out_smc.append(1 if env_test_smc.terminal_flag == 3 else 0)
		(pd.DataFrame({'r_4_smc': r_4_smc, 'att_out_smc': att_out_smc}).
		 to_csv(mc_path + 'monte_carlo_test_smc.csv', sep=',', index=False))
