...
'''RL 学出来的'''
att_ctrl_param.k1 = np.array([27.27148533, 8.755850196, 17.12266326])
att_ctrl_param.k2 = np.array([0.218413067, 0.024192868, 0.03684496])
att_ctrl_param.alpha = np.array([2.5, 2.5, 2.5])
att_ctrl_param.beta = np.array([0.99, 0.99, 0.99])
att_ctrl_param.gamma = np.array([2.528099537, 2.528099537, 2.528099537])
att_ctrl_param.lmd = np.array([2.746498823, 2.746498823, 2.746498823])
'''RL 学出来的'''
...
if __name__ == '__main__':
    ...
    testPath0 = os.path.dirname(os.path.abspath(__file__)) + '/../../datasave/nets/position_tracking/pos_debug' + s + '/'
    testPath_smc = None
    ...
    if CREATE_FILE:
        ...
        if CONTROLLER == 'RL':
            (pd.DataFrame({'r': r, 'att_out': att_out, 'pos_out': pos_out}).
             to_csv('./uniform_comparative_pos/uniform_mc_test_rl_pos_debug' + s + '.csv', sep=',', index=False))
        else:
            (pd.DataFrame({'r': r, 'att_out': att_out, 'pos_out': pos_out}).
